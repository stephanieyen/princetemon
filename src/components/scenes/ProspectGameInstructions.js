...
class ProspectGameInstructions extends Scene {
    constructor() {
        ...
        fontLoader.load(
            ...
            function(font) {
                ...
                textMesh.position.set(-0.8, 0.25, 0.1);
                // Cannot use this.add since inside new function
                Scenes.scenes['prospectgameinstructions'].add(textMesh);
            }
        );
        fontLoader.load(
            PixelFont,
            function(font) {
                const geometry = new TextGeometry(
                "Press the arrow keys when they align!",
                    {
                        font: font,
                        size: 0.05,
                        height: 0
                ...
                const textMesh = new Mesh(geometry, new MeshPhongMaterial({color: 0xffffff}));
                textMesh.position.set(-0.6, -0, 0.1);
                // Cannot use this.add since inside new function
                Scenes.scenes['prospectgameinstructions'].add(textMesh);
            }
        );
        fontLoader.load(
            ...
            function(font) {
                const geometry = new TextGeometry(
                "If you let one arrow through, you have to start over!",
                    {
                        font: font,
                        size: 0.05,
                        height: 0
                ...
                const textMesh = new Mesh(geometry, new MeshPhongMaterial({color: 0xffffff}));
                textMesh.position.set(-0.85, -0.12, 0.1);
                // Cannot use this.add since inside new function
                Scenes.scenes['prospectgameinstructions'].add(textMesh);
            }
        );
        fontLoader.load(
            ...
            function(font) {
                const geometry = new TextGeometry(
                "Press space to continue and space again to start!",
                    {
                        font: font,
                        size: 0.05,
                        height: 0
        ...
        this.pressSpaceBar = (event) => {
            if (event.key === ' ' || event.code === 'Space') {
                Scenes.switchScene('prospectgame');
            }
...
export default ProspectGameInstructions;